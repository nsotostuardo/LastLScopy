...
def get_poisson_estimates(bins, bins_float, SNFinalPos, SNFinalNeg, LimitN, MinSN, OutputPath):
	...
	for sn in bins:
		n_pos_cum = np.count_nonzero(SNFinalPos >= sn)
		n_neg_cum = np.count_nonzero(SNFinalNeg >= sn)
		n_pos_bin = np.count_nonzero((SNFinalPos >= sn) & (SNFinalPos < sn + 1))
		n_neg_bin = np.count_nonzero((SNFinalNeg >= sn) & (SNFinalNeg < sn + 1))

		if n_pos_cum > 0:
			Fraction,FractionE1,FractionE2 = GetPoissonErrorGivenMeasurements(n_neg_cum, n_pos_cum)
		...
		if n_pos_bin > 0:
			Fraction,FractionE1,FractionE2 = GetPoissonErrorGivenMeasurements(n_neg_bin, n_pos_bin)
			if Fraction>1.0:
				Fraction = 1.0
				FractionE1 = 0.0
				FractionE2 = 0.0
		...
		NPositive.append(1.0 * n_pos_cum)
	...
	print('Min SN to do the fit:',round(MinSNtoFit / 10.0,1),', Number of usable bins:',UsableBins)
	AuxiliarOutput.write(str(round(MinSNtoFit / 10.0,1))+' ' + str(UsableBins)+'\n')
	if UsableBins<6:
	...
	while UsableBins>6:
		MinSNtoFit = MinSNtoFit + 1
		UsableBins = len(Nnegative[bins>=MinSNtoFit][Nnegative[bins>=MinSNtoFit]>LimitN])
		print('Min SN to do the fit:',round(MinSNtoFit / 10.0,1),', Number of usable bins:',UsableBins)
		AuxiliarOutput.write(str(round(MinSNtoFit / 10.0,1))+' ' + str(UsableBins)+'\n')

		if MinSNtoFit > int(bins[-1]):
	...
	if UsableBins>=3:
		try:
			...
			popt, pcov = curve_fit(NegativeRateLog, 
					bins_float[bins>=MinSNtoFit][Nnegative[bins>=MinSNtoFit]>LimitN],
	...
	NegativeFitted = NegativeRate(bins_float,popt[0],popt[1])
