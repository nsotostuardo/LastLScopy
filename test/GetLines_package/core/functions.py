...
def spectral_sigma_values(args):
	raw = getattr(args, 'SpectralSigmas', '')
	if raw is None or str(raw).strip() == '':
		return list(range(args.MaxSigmas))
	...
	if len(values) == 0:
		raise ValueError('*** SpectralSigmas must include at least one sigma value ***')
	if any(value < 0 for value in values):
		raise ValueError('*** SpectralSigmas cannot contain negative sigma values ***')
	if values != sorted(set(values)):
		raise ValueError('*** SpectralSigmas must be unique and sorted increasingly ***')
	return values

def spectral_sigma_count(args):
	return len(spectral_sigma_values(args))


def pixels_BMAJ(args):
	if args.UserEPS=='False':
		PixelsPerBMAJ = GetPixelsPerBMAJ(args.Cube)
		if spectral_sigma_count(args) == 1:
			PixelsPerBMAJ = 1.0
...
def get_LEE_factor(args):
	n_sigmas = spectral_sigma_count(args)
	if n_sigmas > 1:
		FactorLEE = 1.0 + (np.log(n_sigmas-1.0) + 1.0/(2.0*(n_sigmas-1)) + 0.577)
	else:
		FactorLEE = 1.0
	return FactorLEE
