...
def gaussian_filtering( data:NDArray[np.float64], sigma:float, spatial_sigma:float,  truncate:float=4.0)-> NDArray[np.float64]:

    data = np.asarray(data, dtype=np.float32)
    tmp = np.empty_like(data)

    kernel_z  = _1Dgaussian_kernel(sigma, truncate).astype(np.float32, copy=False)
    kernel_xy = _1Dgaussian_kernel(spatial_sigma, truncate).astype(np.float32, copy=False)
...
def _1Dgaussian_kernel( sigma:float, truncate:float=4.0) -> np.ndarray: 
    """ 
